				    ...
				    u8 *data, int len)
{
	...
	if (hub_settings->shub_out.secondary_page) {
				    ...
				    u8 mask, u8 val)
{
	...
	err = regmap_update_bits(hw->regmap, addr, mask,
				 ST_LSM6DSX_SHIFT_VAL(val, mask));
					 ...
					 bool enable)
{
	...
	hub_settings = &hw->settings->shub_settings;
	if (hub_settings->master_en.secondary_page) {
		err = st_lsm6dsx_set_page(hw, true);
		if (err < 0)
			goto out;
	}
...
{
	const struct st_lsm6dsx_shub_settings *hub_settings;
	int err, depth = len & ST_LS6DSX_READ_OP_MASK;
	struct st_lsm6dsx_hw *hw = sensor->hw;
	u8 config[2], slv_addr, slv_config;

	hub_settings = &hw->settings->shub_settings;
	slv_addr = ST_LSM6DSX_SLV_ADDR(0, hub_settings->slv0_addr);
	slv_config = ST_LSM6DSX_SLV_CONFIG(0, hub_settings->slv0_addr);
	...
	err = st_lsm6dsx_shub_write_reg_with_mask(hw, slv_config,
	...
	err = st_lsm6dsx_shub_read_reg(hw, hub_settings->shub_out.addr, data,
				       depth);

	st_lsm6dsx_shub_master_enable(sensor, false);

	return st_lsm6dsx_shub_write_reg_with_mask(hw, slv_config,
						   ST_LS6DSX_READ_OP_MASK, 0);
}
...
int st_lsm6dsx_shub_probe(struct st_lsm6dsx_hw *hw, const char *name)
{
	...
	u8 config[2], data, slv_addr, slv_config;
